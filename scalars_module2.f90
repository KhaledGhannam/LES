...
use sim_param,only:u,v,w,theta,qmix,path, &
...
!          theta_mean=T_s_min-2._rprec !T_s_min is dimensional while T_s is non-dimensional
           ...
           q_mean=q_init
! SKS
! SOLVED: What ? T_s_min is dimensional while T_s is non-dimensional ? Why ? What is 0.001 ?
! SKS
...
!MM ---
       ...
       if (k_global > (0.33*nproc*(nz-1))) then
...
!close(44)
7781 format('jz, z, ubar, vbar, wbar,T_bar,q_bar:',(1x,I3,1x,F9.4,1x,F9.4,1x,F9.4,1x,F9.4,1x,F9.4,1x,F9.4))
...
do jy=1,ny
write(57,5168) (z_os(i,jy),i=1,nx)
end do
close(57)
