...
real(kind=rprec),dimension(ld,ny,$lbz:nz):: dcodz,dcodx,dcody
...
real(kind=rprec),dimension(ld,ny,$lbz:nz):: RHS_cof,RHS_co
real(kind=rprec), dimension(ld,ny,$lbz:nz):: sgs_co3,sgs_co1,sgs_co2 ! defines the surface sgs flux

real(kind=rprec),dimension(ld,ny)::co_s_filtered ! filtered T_s for calc of wT_s


real(kind=rprec),dimension(ld,ny,$lbz:nz)::cmix_old

contains
!----------------------XXXXXXXXXXXVIJXXXXXXXXXXX-----------------------
subroutine co_all_in_one
use topbc,only:sponge
use test_filtermodule
implicit none
real(kind=rprec),dimension(nx,ny)::wco_s_current
...
$else
  $define $lbz 1
$endif
real(kind=rprec),dimension(ld,ny,$lbz:nz)::dcodx,dcody
real(kind=rprec),dimension($lbz:nz)::sponge_co

 if ((.not. USE_MPI) .or. (USE_MPI .and. coord == 0)) then
        wco_s_current(1:nx,1:ny)=wco_s(1:nx,1:ny)
 end if
...
call filt_da(cmix,dcodx,dcody)
call ddz_uv (dcodz,cmix)

if ((.not. USE_MPI) .or. (USE_MPI .and. coord == nproc-1)) then
   dcodz(:,:,Nz)=dcodz_top/co_scale*z_i ! Valid for temperature
end if
...
! based computation in scalar_RHS_calc (calculation of RHS_m)
  call mpi_sendrecv (w(1, 1, 1), ld*ny, MPI_RPREC, down, tag_counter+43,  &
                     ...
                     comm, status, ierr)   
  call mpi_sendrecv (dcodz(1, 1, 1), ld*ny, MPI_RPREC, down, tag_counter+44,  &
                     dcodz(1, 1, nz), ld*ny, MPI_RPREC, up, tag_counter+44,   &
                     comm, status, ierr)   
...
call scalar_co_RHS_calc(cmix,dcodx,dcody,dcodz,co_s,z_os,RHS_co,sgs_co1,sgs_co2,sgs_co3,wco_s_current)
...
if (ubc==1 .and. damping_method==2) then ! add the damping term to the scalar equation
   do jz=1,nz-1
   RHS_co(1:nx,1:ny,jz)=RHS_co(1:nx,1:ny,jz)-0.5_rprec*(sponge(jz)+sponge(jz+1))*&
                 (cmix(1:nx,1:ny,jz)-sum(cmix(1:nx,1:ny,jz))/(nx*ny))
   end do
end if
...
call step_scalar_co(cmix,RHS_co,RHS_cof)
end if



$if ($MPI)
  call mpi_sendrecv (cmix(1, 1, 1), ld*ny, MPI_RPREC, down, tag_counter+20,  &
                     cmix(1, 1, nz), ld*ny, MPI_RPREC, up, tag_counter+20,   &
                     comm, status, ierr)   
  call mpi_sendrecv (cmix(1, 1, nz-1), ld*ny, MPI_RPREC, up, tag_counter+21,  &
                     cmix(1, 1, 0), ld*ny, MPI_RPREC, down, tag_counter+21,   &
                     comm, status, ierr)
$endif

end subroutine co_all_in_one

!----------------------XXXXXXXXXXXVIJXXXXXXXXXXX-----------------------

subroutine scalar_co_RHS_calc(scalar,dsdx,dsdy,dsdz,S_Surf,z_os,RHS,sgs_horx,sgs_hory,sgs_vert,surf_flux_current)
use test_filtermodule
implicit none
$if ($MPI)
  $define $lbz 0
...
real(kind=rprec),dimension(nx,ny):: surf_flux_current,ustar_local,S_Surf,surf_flux,z_os,wco_s2
         ...
         ustar_local(:,:)=ustar_avg(:,:)
          ...
          call test_filter(scalar_node_1,G_test) 
            if (lbc==1.and.scalar(1,1,1)<2) then
                    ...
                    wco_s2(1:nx,1:ny)=wco_s(1:nx,1:ny)
                    do i=1,Nx
                    do j=1,Ny
                    surf_flux(i,j)=wco_s2(i,j)/co_scale/u_star
                    end do
                    end do
...
end subroutine scalar_co_RHS_calc
!----------------------XXXXXXXXXXXVIJXXXXXXXXXXX-----------------------
!----------------------XXXXXXXXXXXVIJXXXXXXXXXXX-----------------------
subroutine step_scalar_co(scalar_co,RHS_pre_co,RHS_post_co)
implicit none
integer:: i,j,k
$if ($MPI)
  $define $lbz 0
$else
  $define $lbz 1
$endif
real(kind=rprec),dimension(ld,ny,$lbz:nz)::scalar_co, RHS_pre_co, RHS_post_co

do k=1,nz-1
      do j=1,ny
             do i=1,nx
                 scalar_co(i,j,k)= scalar_co(i,j,k)+dt*(1.5_rprec*RHS_pre_co(i,j,k)-0.5_rprec*RHS_post_co(i,j,k))
             end do
      end do
end do     
...
if ((.not. USE_MPI) .or. (USE_MPI .and. coord == nproc-1)) then
     if ((L_z*nproc)>z_i) then ! for temperature and non-neutral case
         scalar_co(:,:,Nz)=scalar_co(:,:,Nz-1)+dcodz_top/co_scale*z_i*dz !ubc 
! inv_strength - refers to the slope of the inversion ~ 0.003K/Km for temperature
         ...
         scalar_co(:,:,Nz)=scalar_co(:,:,Nz-1)
     end if
end if

end subroutine step_scalar_co


end module scalars_module_co
