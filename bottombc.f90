...
real(kind=rprec),parameter::theta_s1=290._rprec,q_s1=12._rprec,co_s1=10.0_rprec

real(kind=rprec),dimension(nx,ny)::ustar_avg
! This is the common local ustar computed in obukhov and used in wallstress.f90
real(kind=rprec),dimension(nx,ny)::zo,T_s,q_s,co_s
real(kind=rprec),dimension(nx,ny)::z_os
real(kind=rprec),dimension(nx,ny)::zom,wt_s,wq_s,wco_s
...
!open(unit=9004,file='./wcs.dat',status='unknown')
...
! end if
   ...
   do i=1,nx
     ...
     co_s(i,j)=co_s1/co_scale
     z_os(i,j)=0.005/(z_i*7.39_rprec)
     wt_s(i,j)=0.0/(rho_d*cp)
     wq_s(i,j)=1000.0_rprec*0.0/(rho_d*Le)   ! multiply by 1000 for g/kg humidity
     wco_s(i,j)=1000.0_rprec*0.0
