...
if(initu)then         ! if_initu_begins
  if(initsc) then     ! if_initsc_begins
    ...
    if (coord == 0) then
    print *,'Reading initial velocity and scalars from file'
    end if
...
$if ($MPI)
  ...
  call mpi_sendrecv (cmix(1, 1, nz-1), ld*ny, MPI_RPREC, up, 329,  &
                     cmix(1, 1, 0), ld*ny, MPI_RPREC, down, 329,   &
                     comm, status, ierr)

  call mpi_sendrecv (cmix(1, 1, 1), ld*ny, MPI_RPREC, down, 330,  &
                     cmix(1, 1, nz), ld*ny, MPI_RPREC, up, 330,   &
                     comm, status, ierr)
$endif
...
real(kind=rprec)::rms, noise, arg, arg2,theta_mean,q_mean,co_mean
...
! SKS
real(kind=rprec),dimension(nz_tot-1)::utotinit,vtotinit,wtotinit,Ttotinit,qtotinit,ctotinit
real(kind=rprec),dimension(nz-1)::uinit,vinit,winit,Tinit,qinit,cinit
...
! SKS
       ...
       if (lbc .eq. 0) then
         ...
         if (coord == 0) then
         print *,'theta_mean = , q_mean= , co_mean= ',theta_mean, q_mean, co_mean
         end if
...
!c...Taking std dev of vel as 1 at all heights
       ...
       if (z .LE. z_turb_limit) then
         ...
         noise=rms/0.289_rprec*(ran3(seed)-0.5_rprec)
         cmix(jx,jy,jz)=(co_mean+10._rprec*noise*(1-z/z_i)*q_star)/co_scale 
         ...
         cmix(jx,jy,jz)=(co_mean+(z-z_inv)*inv_strength_co)/co_scale
  ...
  if ((.not. USE_MPI) .or. (USE_MPI .and. coord == nproc-1)) then
    ...
    cmix(1:nx, 1:ny, nz) = cmix(1:nx, 1:ny, nz-1)+dcodz_top/co_scale*z_i*dz
   end if
...
! SKS
     ...
     float(nx*ny)),(sum(w(:,:,jz))/float(nx*ny)),&
     (sum(theta(:,:,jz))/float(nx*ny))*T_scale,(sum(qmix(:,:,jz))/float(nx*ny))*q_scale,&
     (sum(cmix(:,:,jz))/float(nx*ny))*co_scale

end do

7781 format('coord, jz, z, ubar, vbar, wbar,T_bar,q_bar,co_bar:',&
             (1x,I3,1x,I3,1x,F9.4,1x,F9.4,1x,F9.4,1x,F9.4,1x,F9.4,1x,F9.4,1x,F9.4))
...
do jz=1,nz-1      
   ...
   cinit(jz) = (sum(cmix(:,:,jz))/float(nx*ny))*co_scale
end do
...
$if ($MPI) 
  ...
  recvcounts = size(cinit)
  displs = coord_of_rank * recvcounts
  call mpi_gatherv (cinit(1),size(cinit),MPI_RPREC, &
                    ctotinit(1),recvcounts,displs,  &
                    MPI_RPREC,rank_of_coord(0),comm,ierr)
$else
  utotinit = uinit
  vtotinit = vinit
...
if((.not. USE_MPI) .or. (USE_MPI .and. coord == 0)) then
  do jz=1,nz_tot-1
     write(103,8001) utotinit(jz),vtotinit(jz),wtotinit(jz),Ttotinit(jz),qtotinit(jz),ctotinit(jz)
  end do
  write(103,*)
end if
